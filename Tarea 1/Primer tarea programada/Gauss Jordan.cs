...
namespace Primer_tarea_programada
{
    class Gauss_Jordan
    {
        ...
        public static List<List<float>> dividirMatrizAumentada(List<List<float>> matrizAumentada)
        {
            int x = matrizAumentada[0].Count/2;
            for (int i = 0; i < matrizAumentada.Count; i++)
            {
                for (int y = 0; y < x; y++)
                {
                    matrizAumentada[i].RemoveAt(0);
                }
            }
        ...
        {
            LOG += ("divide la fila " + (fila + 1).ToString() + " entre " + divisor.ToString()+ "\n");
            for (int i = 0; i < matrizAumentada.Count; i++)
            {
        ...
        {
            LOG+=("Resta " + numero.ToString() + " veces la fila " + (filaUno + 1).ToString() + " a la fila " + (filaModificar + 1).ToString()+"\n");
        ...
        {
            LOG = "Imprimiendo matriz Aumentada";
            ...
            for (int col = 0; col < matrizAumentada[0].Count / 2; col++)
            {
                if (matrizAumentada[col][col] == 0)
                {
                    ...
                    while (true)
                    {
                        if (xcambio == col)
                        {
                            LOG += ("Matriz invalidada: columna de 0 \n");
                            return matrizAumentada;
                        }
                ...
                else
                {
                    ...
                    if (matrizAumentada[col][col] != 1)
                    {
                        ...
                        //LOG += ("\n (" + (col + 1).ToString() + "," + (1 + col).ToString() + ")" + ":" + matrizAumentada[col][col]+"\n");
                        imprimir(matrizAumentada);
                    }
                }
                for (int fil = 0; fil < matrizAumentada.Count; fil++)
                {
                    ...
                    if (fil != col)
                    {
                        if (matrizAumentada[fil][col] != 0)
                        {
                            ...
                            //LOG += ("\n (" + (fil + 1).ToString() + "," + (1 + col).ToString() + ")" + ":" + matrizAumentada[fil][col]+"\n");
                            imprimir(matrizAumentada);
                        }
                    }
                }
            }
            LOG += "\n Se ha calculado la inversa";
            return matrizAumentada;
        }
